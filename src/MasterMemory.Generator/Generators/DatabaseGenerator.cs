...
internal static class DatabaseGenerator
{
        ...
        using (sb.BracketScope())
        {
            using (sb.Append("this._container = new ").Append(TableContainerGenerator.Name).Scope('('))
            {
                sb.AppendJoin(",",
                    ...
                    (sb, model) =>
                    {
                        using (sb.Append("new ").Append(model.TableName).Scope('('))
                        {
        ...
        bool isInterface)
    {
        foreach (TableModel model in tableArray)
        {
            sb.Append("public I").Append(model.TableName).Append(' ').Append(model.TableName);
            if (isInterface)
            {
                sb.AppendLine(" { get; }");
            }
...

        {
            ...
            using (sb.BracketScope())
            {
                ...
                foreach (TableModel model in tableArray)
                {
                    sb.Append("dict.Add(\"").Append(model.TableRawName).Append("\", ").Append(model.TableFullName)
